...
async def get_social(area: str = Query("g10", description="Area code")):
    ...
    if "g10" in area_lower or "g-10" in area_lower:
        ...
        # Unknown area — return structured response that helps AI reason about the gap
        return {
            "area": area,
            "signals": [],
            "total_mentions": 0,
            ...
            "reasoning_hint": (
                f"No social media monitoring is configured for '{area}'. "
                "This does NOT mean no crisis exists — it means social signals "
                "cannot be verified for this area. The AI should rely on other "
                "available data sources (weather, traffic, sensors) for analysis. "
                "Absence of social data should not reduce crisis confidence if "
