...
class FirebaseClient:
    ...
    def is_connected(self) -> bool:
        """Check whether Firebase SDK initialized and client is not in mock mode."""
        return self.db is not None and not self.use_mock

    def has_firestore_access(self) -> bool:
        """
        Verify real Firestore access with a lightweight read.

        This avoids false positives where SDK initialization succeeds
        but service-account auth is invalid at request time.
        """
        if not self.is_connected():
            return False

        try:
            # Bounded write+read to validate auth/access without hanging.
            health_ref = self.db.collection("agent_traces").document("_healthcheck")
            health_ref.set({"status": "ok"}, merge=True, timeout=15)
            _ = health_ref.get(timeout=15)
            return True
        except Exception as e:
            print(f"[FirebaseClient] Firestore access check failed: {e}")
            return False
    ...
    def get_agent_trace(self, trace_id: str) -> Optional[dict]:
        """
        Retrieve a specific agent trace by trace ID.

        Returns:
            Agent trace dictionary or None if not found.
        """
        try:
            if self.use_mock:
                return self._mock_store["agent_traces"].get(trace_id)

            doc = self.db.collection("agent_traces").document(trace_id).get()
            return doc.to_dict() if doc.exists else None
        except Exception as e:
            print(f"[FirebaseClient] Failed to get agent trace {trace_id}: {e}")
            return None
