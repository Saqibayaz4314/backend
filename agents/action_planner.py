...
class ActionPlanner:
    ...
    def _generate_actions_with_gemini(
        ...
        """
        # Extract traffic route data if stored from signal collection
        traffic_context = ""
        try:
            from mock_api.traffic_router import _get_coords, EMERGENCY_DESTINATIONS, _get_city
            coords = _get_coords(crisis.location)
            city = _get_city(crisis.location)
            destinations = EMERGENCY_DESTINATIONS.get(city, [])
            if destinations:
                traffic_context = f"\nNEAREST EMERGENCY FACILITIES: {', '.join(d['name'] for d in destinations[:3])}"
        except Exception:
            pass
        ...
        system_instruction = (
            ...
            "When generating TRAFFIC_REROUTE actions, include specific alternative routes."
        )
...
INSTRUCTIONS:
- For TRAFFIC_REROUTE: Specify the blocked road AND the alternative route to use
- For EMERGENCY_DISPATCH: Specify which hospital/unit and estimated arrival time
- Consider actual distances and traffic conditions in your actions
