...
class CrisisDetector:
    ...
    def _classify_with_gemini(self, signals: SignalCollection) -> dict:
        ...
        system_instruction = (
            ...
            "IMPORTANT: If social media data is not available for an area, do NOT lower "
            "confidence. Instead, rely on other available signals (weather, traffic, sensors). "
            "Absence of social data does not mean absence of crisis."
        )
...
- If social media signals are absent or show "no monitoring configured", do NOT reduce confidence.
- Weather, traffic (especially Google Maps real-time data), and sensor data are equally valid sources.
- If traffic data shows high congestion with specific blocked routes, that IS strong evidence.
- Cross-reference available signals to determine crisis type and confidence.
