...
class SituationAnalyzer:
    ...
    def _analyze_with_gemini(
        ...
        system_instruction = (
            ...
            "IMPORTANT: If social media data is unavailable for an area, rely on traffic "
            "(Google Maps), weather, and sensor data. Do NOT reduce severity just because "
            "social signals are missing."
        )

        # Extract traffic route info if available
        traffic_info = ""
        social_data_status = "available"
        for sig in signals.signals:
            if sig.source.value == "TRAFFIC_API" and sig.metadata:
                routes = sig.metadata.get("routes", [])
                if routes:
                    route_lines = []
                    for r in routes[:3]:
                        route_lines.append(
                            f"  - To {r.get('destination', '?')}: {r.get('distance', '?')}, "
                            f"normal={r.get('duration_normal', '?')}, "
                            f"with traffic={r.get('duration_in_traffic', '?')}, "
                            f"congestion={r.get('congestion_level', '?')}/10"
                        )
                    traffic_info = "\nTRAFFIC ROUTES (from Google Maps):\n" + "\n".join(route_lines)
                    if sig.metadata.get("is_real"):
                        traffic_info += "\n  [Source: Google Maps Real-Time Data]"
                    blocked = sig.metadata.get("blocked_routes", [])
                    if blocked:
                        traffic_info += f"\n  Blocked Routes: {', '.join(blocked)}"

        # Check if social data was unavailable
        for sig in signals.signals:
            if sig.source.value == "SOCIAL_MEDIA" and "no monitoring" in sig.text.lower():
                social_data_status = "NOT AVAILABLE for this area"
...
SOCIAL DATA STATUS: {social_data_status}
...
- If social data is unavailable, use traffic congestion, weather, and sensor data to assess
- Use actual route distances and traffic delays in your reasoning
