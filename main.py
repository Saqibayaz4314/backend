...
# ---------------------------------------------------------------------------
# Demo User Initialization for Testing
# ---------------------------------------------------------------------------
def _initialize_demo_user():
    """Add demo user for hackathon testing/demo purposes."""
    try:
        import bcrypt
        from utils.firebase_client import FirebaseClient
        
        demo_email = "admin@ciro.com"
        demo_password = "ciro123"
        demo_name = "CIRO Admin"
        ...
        demo_user = {
            ...
            "verified": True,  # Pre-verified for demo
            "created_at": datetime.now(timezone.utc).isoformat(),
            "role": "admin",
            "demo_account": True,
        }
        
        # Try to save to Firebase, fall back to in-memory
        try:
            fb = FirebaseClient()
            if fb.is_connected():
                fb.db.collection("users").document(demo_email).set(demo_user, merge=True)
                print(f"✅ [DEMO] User saved to Firebase: {demo_email}")
            else:
                # Fallback to in-memory store
                from auth import _users_store
                _users_store[demo_email] = demo_user
                print(f"✅ [DEMO] User added to in-memory store: {demo_email}")
        except Exception as e:
            # Fallback to in-memory
            from auth import _users_store
            _users_store[demo_email] = demo_user
            print(f"✅ [DEMO] User added (fallback): {demo_email}")
            
        print(f"\n{'='*70}")
        print(f"  DEMO ACCOUNT CREDENTIALS")
        print(f"  {'='*70}")
        ...
        print(f"[DEMO] Warning: Could not initialize demo user: {e}")
...
def _normalize_trace_id(client_trace_id: Optional[str], incident_id: str) -> str:
    """
    Return a safe trace ID.
    If client provides one, sanitize it and keep TRACE- prefix.
    """
    if not client_trace_id:
        return f"TRACE-{incident_id}"

    cleaned = re.sub(r"[^A-Za-z0-9_-]", "-", client_trace_id.strip())
    if not cleaned:
        return f"TRACE-{incident_id}"
    if not cleaned.startswith("TRACE-"):
        cleaned = f"TRACE-{cleaned}"
    return cleaned[:120]
...
async def analyze_crisis(request: AnalyzeRequest):
    ...
    trace_id = _normalize_trace_id(request.client_trace_id, incident_id)
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Step 6: Print pipeline completion
        # ------------------------------------------------------------------
        logger.print_pipeline_complete()

        # ------------------------------------------------------------------
        # Step 7: Build & return AnalyzeResponse
        # ------------------------------------------------------------------
...
async def get_agent_logs(trace_id: str):
    ...
    if not trace_path.exists():
        # Fallback: read from Firebase in case local file is unavailable (e.g., cloud instance)
        firebase = FirebaseClient()
        trace = firebase.get_agent_trace(trace_id)
        if trace:
            return trace
...
async def analyze_multi_crisis(request: MultiCrisisRequest):
    ...
    for idx, result in enumerate(sorted_results):
        ...
        # Print pipeline completion for this crisis
        logger.print_pipeline_complete()
